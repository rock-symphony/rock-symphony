...
class sfCompileConfigHandler extends sfYamlConfigHandler
{
  ...
  public function execute(array $configFiles): string
  {
    ...
    foreach ($config as $file) {
      if (! is_readable($file)) {
        // file doesn't exist
        throw new sfParseException(sprintf('Configuration file "%s" specifies nonexistent or unreadable file "%s".',
          $configFiles[0], $file));
      }
  ...
  static public function getConfiguration(array $configFiles): array
  {
    ...
    array_walk_recursive($config, function (& $path) {
      $path = self::replacePath($path);
    });
